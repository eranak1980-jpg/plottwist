...
def assign(g):
 ...
 killer=random.choice(players);t,v=story(g);place=g['location'] or 'הסלון';j=clean(g['inside_joke']);roles=[('צלם הלילה','צילמת משהו רגע לפני שהאורות כבו.','השתמש במה שראית כדי לבדוק את הסיפורים של האחרים.'),('שומר הסוד',f'{v} סיפר לך שהוא עומד לחשוף מישהו.','גלה למי היה הכי הרבה מה להפסיד.'),('המארח חד־העין',f'אתה זוכר מי נכנס ומי יצא מ{place}.','חפש סתירות בסיפורים של האחרים.'),('החבר שנעלם','נעלמת לזמן קצר מסיבה תמימה.','שמור על האליבי שלך, אלא אם מישהו מאשים אותך.')];random.shuffle(roles)
 with cn() as c:
  c.execute('UPDATE games SET killer_player_id=?,story_title=?,victim_name=? WHERE id=?',(killer['id'],t,v,g['id']))
  for i,p in enumerate(players):
   if p['id']==killer['id']:r='הרוצח' if g['game_type']=='murder' else 'האשם';s=f'אתה אחראי למה שקרה ל{v}. מישהו כמעט ראה אותך ליד {place}.';o='הסט את החשד ממך ושמור על סיפור עקבי.';h=f'רמז פרטי: מישהו זוכר שראה אותך ליד {place}.'
   else:r,s,o=roles[i%len(roles)];h=f'רמז פרטי: המקרה כולל זיכרון שרק החבורה מכירה: "{j}". נסה להבין מי יכול היה להשתמש בו ולמה.' if j else 'רמז פרטי: פרט אישי קטן חשוב יותר ממה שנראה.'
...
def prompt(g):
 n=int(g['round_no']);t=g['story_title'] or 'התעלומה של הערב';v=g['victim_name'] or 'הקורבן';p=g['location'] or 'החדר';j=clean(g['inside_joke']);personal=f'רמז חדש מזכיר זיכרון שרק החבורה שלכם מכירה: "{j}". מי יכול היה להשתמש בפרט הזה ולמה?' if j else 'ברמז החדש מופיע פרט אישי שרק מישהו מהחבורה אמור להכיר.';arr=[f'{t}. אחרי הפסקת חשמל ב{p}, {v} נמצא ללא רוח חיים. כל אחד מציג את הדמות והאליבי שלו — אבל שומר את הסוד לעצמו.',f'{personal} הרמז הפרטי שלך פתוח עכשיו. בחר שחקן אחד ואתגר פרט בסיפור שלו.',f'עדות חדשה סותרת משהו שכבר נאמר. כל אחד מציג במשפט אחד את ציר הזמן שלו ועונה על שאלה ישירה משחקן אחר.',f'הגיע הזמן להאשמה הסופית. אמרו מי לדעתכם אחראי, הסבירו למה, ואז כל אחד מצביע בסוד.'];return arr[n-1] if 1<=n<=4 else 'המשחק הסתיים.'
def left(g):
...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p=='/health':return self.J({'ok':True,'version':'hebrew-playtest'})
 ...
 def do_POST(self):
  ...
  if p=='/api/create':
   ...
   with cn() as c:cur=c.execute('INSERT INTO games(code,host_token,group_name,location,inside_joke,game_type,created_at) VALUES(?,?,?,?,?,?,?)',(co,ht,clean(d.get('group_name'),80) or 'החבורה',clean(d.get('location'),80) or 'הסלון',clean(d.get('inside_joke')),clean(d.get('game_type'),20) or 'murder',now()));c.execute('INSERT INTO players(game_id,name,token,joined_at) VALUES(?,?,?,?)',(cur.lastrowid,name,pt,now()))
  ...
  if act=='next':
   ...
   rn=int(g['round_no'])+1
   with cn() as c:
    if rn>4:c.execute("UPDATE games SET status='finished',round_started_at='' WHERE id=?