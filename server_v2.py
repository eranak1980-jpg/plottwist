...
def start_case(game_id):
 ...
 with cn() as c:
  c.execute("UPDATE games SET status='playing',round_no=1,turn_index=0,killer_player_id=?,story_title=?,victim_name=?,case_json=?,phase_started_at=? WHERE id=?",(culprit['id'],case['title'],case['victim'],json.dumps(case,ensure_ascii=False),now(),game_id))
  for p in players:
...
def state_v2(game_id,token=''):
 ...
 case=json.loads(g['case_json']) if g['case_json'] else None;out={'status':g['status'],'round_no':g['round_no'],'turn_index':int(g['turn_index'] or 0),'title':g['story_title'],'players':[{'id':p['id'],'name':p['name'],'has_photo':bool(p['photo_data']),'has_ai_art':bool(p['ai_art_data'])} for p in players]}
